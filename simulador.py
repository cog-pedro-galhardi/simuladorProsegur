...
def prepare_data(demand_df, escala_df, abs_df):
    ...
    escala_trabalhando = escala_df.merge(
        abs_df[["nome", "tx_absenteismo"]], on=["nome"], how="left"
    )
    escala_trabalhando = escala_trabalhando[
        (escala_trabalhando["marcacao"] == "E")
        & (escala_trabalhando["data"].isin(demand_df["data"]))
    ]
    return escala_trabalhando


def monte_carlo_simulation(
    escala_df, demanda_df, n_simulacoes=1000, tx_var_demanda=0, tx_var_abs=0
):
    """
    ...
    escala_base = escala_df[
        ["data", "cod_filial", "cargo", "nome", "tx_absenteismo"]
    ].copy()
    escala_base["tx_absenteismo"] *= 1 + tx_var_abs

    # Expandir escala_base para todas as simulações
    escala_expanded = escala_base.loc[
        np.repeat(escala_base.index.values, n_simulacoes)
    ...
    presenca = (
        escala_expanded[escala_expanded["presente"]]
        .groupby(["simulacao"] + chave_cols)
        .size()
        .reset_index(name="n_presentes")
    )

    # Expandir demanda_base para todas as simulações
    demanda_expanded = demanda_base.loc[
        np.repeat(demanda_base.index.values, n_simulacoes)
    ].reset_index(drop=True)
    ...
    demanda_simulada = demanda_expanded.merge(
        presenca, on=chave_cols + ["simulacao"], how="left"
    )
    demanda_simulada["n_presentes"] = (
        demanda_simulada["n_presentes"].fillna(0).astype(int)
    )

    # Calcular demanda não atendida
    demanda_simulada["demanda_nao_atendida"] = np.maximum(
        demanda_simulada["quantidade"] - demanda_simulada["n_presentes"], 0
    )
...
def plot_estatisticas(estatisticas, filial, cargo, faixa):
    ...
    """
    dados = estatisticas[
        (estatisticas["cod_filial"] == filial)
        & (estatisticas["cargo"] == cargo)
        & (estatisticas["faixa"] == faixa)
    ].sort_values("data")
...
def simulation_results(resultados_df):
    ...
    df1 = (
        resultados_df.groupby(["mês", "data", "cod_filial", "cargo", "faixa"])
        .agg({"quantidade": "first", "demanda_nao_atendida": "mean"})
        .reset_index()
    )
    ...
    df2 = (
        df1.groupby(["mês", "data", "cod_filial", "cargo"])
        .agg({"quantidade": "sum", "faltaEsperada": "sum"})
        .reset_index()
    )
    ...
    df3 = (
        df2.groupby(["cod_filial", "cargo"])
        .agg({"quantidade": "mean", "faltaEsperada": ["mean", "sum"]})
        .reset_index()
    )
    ...
    # média da soma da Média demanda_nao_atendida, soma da soma da Média demanda_nao_atendida
    df4 = (
        df3.groupby(["cod_filial"])
        .agg(
            {
                "quantidade": "mean",
                "media_faltaEsperada": ["mean"],
                "soma_faltaEsperada": ["sum"],
            }
        )
        .reset_index()
    )
